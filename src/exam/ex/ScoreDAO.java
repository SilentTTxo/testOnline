...
public class ScoreDAO {
	...
	public Score findById(java.lang.Integer id) {
		...
		try {
			Score instance = (Score) getCurrentSession().get("exam.ex.Score",
					id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List findByExample(Score instance) {
		...
		try {
			List results = getCurrentSession().createCriteria("exam.ex.Score")
	...
	public Score merge(Score detachedInstance) {
		...
		try {
			Score result = (Score) getCurrentSession().merge(detachedInstance);
